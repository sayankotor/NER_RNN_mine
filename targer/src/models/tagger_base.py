...
class TaggerBase(nn.Module):
    ...
    def __init__(self,  word_seq_indexer, tag_seq_indexer, gpu, batch_size, useElmo = False, weight_file = '', options_file = ''):
        super(TaggerBase, self).__init__()
        print ("init targer base")
        print (useElmo)
        print (weight_file)
        print (options_file)
        ...
        self.useElmo = useElmo
        if (useElmo):
            self.elmo = Elmo(options_file, weight_file, 1, dropout=0)
            print ("elmo is initiated")
